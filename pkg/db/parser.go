...
func GetLabelNamesFromQuery(conn *sql.Conn, query Sql) []string {
	parseSql := Sql(`select coalesce(nullif(row->>'alias', ''), row->>'$.column_names[-1]') from (select unnest::json as row from unnest(json_serialize_sql('` + string(query) + `')->>'$.statements[0].node.select_list[*]'));`)
